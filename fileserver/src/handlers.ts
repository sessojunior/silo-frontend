 ...
 */
export const handleMainUpload: ExpressHandler = async (req, res) => {
  try {
    const file = req.file as MulterFile | undefined
    if (!file) {
      const errorResponse: ErrorResponse = { success: false, error: 'Nenhum arquivo enviado' }
    ...
    if (!isValidType) {
      const errorResponse: ErrorResponse = { success: false, error: 'Tipo de arquivo não permitido' }
      res.status(400).json(errorResponse)
      return
    }
 ...
 */
export const handleContactUpload: ExpressHandler = async (req, res) => {
  try {
    const file = req.file as MulterFile | undefined
    if (!file) {
      const errorResponse: ErrorResponse = { success: false, error: 'Nenhum arquivo enviado' }
 ...
 */
export const handleProblemUpload: ExpressHandler = async (req, res) => {
  try {
 ...
 */
export const handleSolutionUpload: ExpressHandler = async (req, res) => {
  try {
    const files = req.files as MulterFile[] | undefined
    if (!files || files.length === 0) {
      const errorResponse: ErrorResponse = { success: false, error: 'Nenhum arquivo enviado' }
