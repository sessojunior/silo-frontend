...
export default function ProblemsPage() {
	...
	useEffect(() => {
		const fetchProblems = async () => {
			...
			try {
				...
				const mappedProblems = data.items.map((item: any) => ({
					id: item.id,
					title: item.title,
				...
				const newProblems = data.items.length === 1 ? mappedProblems : [...problems, ...mappedProblems]
				setProblems(newProblems)
				setHasMore(mappedProblems.length === data.items.length)

				// Calcular soluções para cada problema
				const counts: Record<string, number> = {}
				for (const problem of newProblems) {
					const solutionsRes = await fetch(`/api/products/solutions?problemId=${problem.id}`)
					if (solutionsRes.ok) {
						const solutionsData = await solutionsRes.json()
						counts[problem.id] = solutionsData.solutions?.length || 0
					}
				}
	...
	async function handleSolutionSubmit(e: React.FormEvent) {
		...
		try {
			...
			const data = await res.json()
			if (res.ok) {
				toast({ type: 'success', title: solutionMode === 'edit' ? 'Solução atualizada' : 'Solução cadastrada' })
				closeSolutionModal()
				...
				toast({ type: 'error', title: 'Erro', description: data.message || 'Erro ao salvar solução.' })
			}
		} catch (e) {
	...
	return (
		<>
			...
			{/* Modal de solução */}
			<Modal isOpen={solutionModalOpen} onClose={closeSolutionModal} title={solutionMode === 'edit' ? 'Editar solução' : solutionMode === 'reply' ? 'Responder solução' : 'Adicionar solução'}>
				<form className='flex flex-col gap-6 p-6' onSubmit={handleSolutionSubmit}>
					...
					<div className='flex flex-col gap-2'>
						...
						{solutionMode === 'edit' && editingSolution?.image && editingSolution.image.image && !solutionImagePreview ? (
							<div className='flex flex-col items-center justify-center h-32 w-32 border-2 border-dashed border-blue-400 rounded-lg relative'>
					...
					<div className='flex justify-between gap-2'>
						...
						{solutionMode === 'edit' && editingSolution?.isMine && (
							<Button type='button' style='bordered' className='text-red-600 border-red-200 hover:bg-red-50 dark:border-red-800 dark:hover:bg-red-900/20' onClick={() => openDeleteSolutionDialog(editingSolution)}>
								Excluir solução
							</Button>
						)}
						<div className='flex gap-2'>
							...
							<Button type='submit' disabled={solutionLoading}>
								{solutionLoading ? (solutionMode === 'edit' ? 'Salvando...' : solutionMode === 'reply' ? 'Respondendo...' : 'Adicionando...') : solutionMode === 'edit' ? 'Salvar' : solutionMode === 'reply' ? 'Responder' : 'Adicionar'}
							</Button>
						</div>
					</div>
				</form>
			...
			>
				<div className='flex gap-2 justify-end mt-6'>
					...
					<Button type='button' className='bg-red-600 text-white hover:bg-red-700' disabled={deleteSolutionLoading} onClick={confirmDeleteSolution}>
