...
export default function ProblemsPage() {
	...
	useEffect(() => {
		const fetchProblems = async () => {
			...
			if (data.items[0]) {
				const solutions = await fetch(`/api/products/${slug}/solutions/${data.items[0].id}`)
				const solutionsData = await solutions.json()
				setSolutions(solutionsData.items)

				const images = await fetch(`/api/products/${slug}/images/${data.items[0].id}/images`)
		...
		<div className='flex w-full'>
			...
			<div className='flex w-full flex-shrink-0 flex-col border-r border-zinc-200 sm:w-[480px] dark:border-zinc-700'>
				<div className='scrollbar size-full h-[calc(100vh-131px)] overflow-y-auto'>
					...
					{problems.length > 0 ? (
						<ListProblems problems={problems} solutionsCount={solutionsCount} />
					) : (
...
function ListProblems({ problems, solutionsCount }: { problems: ProductProblem[]; solutionsCount: Record<string, number> }) {
	return (
		<div className='flex flex-col'>
			{problems.length > 0 &&
				problems.map(({ id, title, description }: ProductProblem) => (
					<div key={id} className='flex flex-col border-b border-zinc-200'>
						<div className='flex w-full flex-col gap-y-1 p-8 hover:bg-zinc-100'>
							<div className='flex w-full items-center justify-between gap-x-2'>
								...
								<span className='ms-1 shrink-0 rounded-full bg-zinc-100 px-1.5 py-0.5 text-xs font-medium text-zinc-600'>{solutionsCount[id] ?? 0}</span>
							</div>
