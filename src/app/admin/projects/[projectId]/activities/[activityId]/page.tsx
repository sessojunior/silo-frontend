...
export default function TaskKanbanPage() {
	...
	// Função para mover tarefa no Kanban
	const handleTaskMove = useCallback(
		async (taskId: string, fromStatus: string, toStatus: string, overId?: string) => {
			console.log('🔵 Movendo tarefa:', { taskId, fromStatus, toStatus, overId })
			...
			try {
				const response = await fetch(`/api/projects/${projectId}/activities/${activityId}/tasks`, {
					...
					body: JSON.stringify({
						taskId,
						newStatus: toStatus,
						newSort: 0, // Por enquanto sempre 0, depois implementamos reordenação
					}),
				})
				...
				// Atualizar estado local das tasks originais
				setTasks((prevTasks) => prevTasks.map((task) => (task.id === taskId ? { ...task, status: toStatus as ProjectTask['status'], sort: 0 } : task)))

				// Atualizar estado das kanban tasks
				setKanbanTasks((prevTasks) => prevTasks.map((task) => (task.id === taskId ? { ...task, status: toStatus as KanbanTask['status'], sort: 0 } : task)))
				...
				throw error // Re-throw para o KanbanBoard reverter a mudança otimista
			}
		},
		[projectId, activityId],
	)
	...
	return (
		<div className='relative w-full h-[calc(100vh-64px)] flex flex-col overflow-x-auto overflow-y-auto'>
			...
			<div className='flex-1 bg-zinc-50 dark:bg-zinc-900'>
				<KanbanBoard tasks={kanbanTasks} onTaskMove={handleTaskMove} />
			</div>
		</div>
	)
}
