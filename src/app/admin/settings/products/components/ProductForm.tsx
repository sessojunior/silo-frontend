...
interface ProductFormProps {
	initialData?: { id?: string; name: string; available: boolean }
	onSubmit: (data: { id?: string; name: string; available: boolean }) => Promise<void>
	onCancel: () => void
	loading: boolean
}
...
export default function ProductForm({ initialData, onSubmit, onCancel, loading }: ProductFormProps) {
	...
	async function handleSubmit(e: React.FormEvent) {
		...
		setError(null)
		try {
			await onSubmit({ id: initialData?.id, name: name.trim(), available })
