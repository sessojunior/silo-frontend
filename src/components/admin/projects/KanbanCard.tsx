...
export default function KanbanCard({ activity, onEdit }: KanbanCardProps) {
	...
	return (
		<div ref={setNodeRef} style={style} {...attributes} {...listeners} className={`group bg-white dark:bg-zinc-900 rounded-lg border border-zinc-200 dark:border-zinc-700 p-4 cursor-grab transition-all duration-200 hover:shadow-md ${isDragging ? 'opacity-50 shadow-lg' : ''} relative`}>
			...
			{/* Título da Tarefa */}
			<h4 className='font-medium text-zinc-900 dark:text-zinc-100 text-base leading-tight line-clamp-2 mb-3' title={activity.name}>
				{activity.name}
			</h4>

			{/* Descrição */}
			{activity.description && (
				<p className='text-sm text-zinc-600 dark:text-zinc-400 mb-3 line-clamp-4' title={activity.description}>
					{activity.description}
				</p>
			)}
			...
			{activity.category && (
				<div className='flex items-center gap-1 text-xs text-zinc-400 dark:text-zinc-600 mb-3'>
					<span className='uppercase px-2 py-1 bg-zinc-100 text-zinc-400 dark:text-zinc-600 rounded-md'>{activity.category}</span>
				</div>
			)}
			...
			<div className='flex items-center justify-between ml-0.5'>
				...
				{activity.assignees.length > 0 ? (
					<div className='flex -space-x-2'>
						...
						{activity.assignees.length > 3 && <div className='size-8 rounded-full bg-zinc-400 border-2 border-white dark:border-zinc-900 flex items-center justify-center text-xs font-medium text-white'>+{activity.assignees.length - 3}</div>}
					</div>
				) : (
					<div>
